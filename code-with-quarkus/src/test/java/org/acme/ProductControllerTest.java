...
public class ProductControllerTest {
    ...
    public void testUpdateProductEndPoint() {
        ...
        given().when().body(product).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).put("/product/5")
                .then().statusCode(404);
        // -- Update Product 3 with invalid values
        given().when().body(product).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).put("/product/3")
                .then().statusCode(400);
        product.setName("P");
        given().when().body(product).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON).put("/product/3")
                .then().statusCode(400);
